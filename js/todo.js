...
function createTodo(newInput) {
    const todo = `<div class="todo" onclick="todoClick(event)">
                    ...
                    <span class="arrow" onclick="moveTodoUp(this)"><span class="material-symbols-outlined">keyboard_arrow_up</span></span>

                    <div class="arrow" onclick="moveTodoDown(this)"><span class="material-symbols-outlined">keyboard_arrow_down</span></div>

                    <span class="arrow delete" onclick="deleteTodo(this)"><span class="material-symbols-outlined">delete</span></span>
                </div>`;
...
function todoClick(event) {
    let clickedElement = event.target.closest(".todo");

    if (event.target.classList.contains("arrow") || event.target.classList.contains("material-symbols-outlined")) {
        return;
    }

    let checkbox = clickedElement.querySelector(".check input[type='checkbox']");
...
function deleteTodo(arrow) {
    const checkedTodos = document.querySelectorAll(".todo.todo-checked");
    ...
    if (checkedTodos.length !== 0) {
        ...
        if (indexToDelete !== -1) {
            ...
            console.log("Todo not found in the array");
        }
    }
